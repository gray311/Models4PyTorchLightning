...
def window_partition(x, window_size):
    ...
    Returns:
        windows: (num_windows*B, window_size, window_size, C)  # 这里是把一个batch的图片拆成的token全部
    """
    B, H, W, C = x.shape
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.dim = dim  # 按照swin-T里的C，这里等于96
        self.window_size = window_size  # Wh, Ww 7*7，每个窗口包含49个patch（tokens），这些token做SA
        self.num_heads = num_heads
        head_dim = dim // num_heads  # transformer结构，每个head加起来要等于原token的长度，所以要先做低维projection
        self.scale = qk_scale or head_dim**-0.5  #开根号

        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1),
                        num_heads)
        )  # 2*Wh-1 * 2*Ww-1, nH  每个head都要做一遍softmax(QK^T/d + B_i)，所以有num_heads个B_i
    ...
    def forward(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape  # batchsize * (w / windowssize) * (h / windowssize) , 7 * 7 , [96,192,384,768]

        qkv = self.qkv(x).reshape(
            B_, N, 3, self.num_heads, C // self.num_heads
        ).permute(
            2, 0, 3, 1, 4
        )  #话说这样qkv不是不一样了吗？ 答，相当于q k v已经投影过了，原本是q k v单独使用linear层投影，现在是用一个linear投影
        q, k, v = qkv[0], qkv[1], qkv[
            2]  # make torchscript happy (cannot use tensor as tuple)

        q = q * self.scale  # QK^T / d^0.5 防止过小导致softmax权重过小或者过大
        attn = (q @ k.transpose(-2, -1))  # 最后两维换一下
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.register_buffer("attn_mask", attn_mask)  #不会被更新，但是会保存在statelist中
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.reduction = nn.Linear(4 * dim, 2 * dim,
                                   bias=False)  #想到与下采样，我之前一直以为是用卷积层来着
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.proj = nn.Conv2d(
            ...
            stride=patch_size)  # 把图片切成patch，再把每个patch变成token，只需要一个卷积，妙啊
